...
if __name__ == "__main__":
    ...
    contrac = contracts.filter(check_contracts)
    ...
    t = transac.map(mapping1)
    t_reducing = t.reduceByKey(lambda x, y: (x[0]+y[0], x[1]+y[1]))
    avg = t_reducing.sortByKey(ascending=True)
    ...
    t1 = transac.map(mapping2)
    cr= contrac.map(lambda x: (x.split(',')[0],1))
    joins = t1.join(cr)
    mapp = joins.map(lambda x: (x[1][0][0], (x[1][0][1],x[1][1])))
    t_reducing1 = mapp.reduceByKey(lambda x, y: (x[0]+y[0], x[1]+y[1]))
